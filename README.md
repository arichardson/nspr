...
`nspr` avoids this automatically in a single `nspr diff` run:
- **Re-attaching onto an ancestor branch** (e.g. pointing a PR whose base was `main` back onto its parent PR branch in the stack): when the target base branch's current tip is already an ancestor of the PR's current head, `nspr` updates the PR's base branch *before* `git push`, which shrinks the displayed diff immediately and pushes all branches directly onto their new tips in a **single `git push`**.
- **Retargeting onto a branch that has moved ahead** (e.g. `nspr diff --cherry-pick` onto a `main` branch with new upstream commits): `nspr` **parks** the branch at `merge_base(old_base, new_base)` (an ancestor of both), pushes, changes the PR's base branch, and then **immediately catches the branch up** onto the new base tip in the same `nspr diff` invocation. At every step the displayed diff contains only the layer's own files, and you never need to run `nspr diff` twice.
